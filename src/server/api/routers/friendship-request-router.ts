...
export const friendshipRequestRouter = router({
  ...
  accept: procedure
    ...
    .mutation(async ({ ctx, input }) => {
      await ctx.db.transaction().execute(async (t) => {
        ...
        /**
         * Implement api to accept a friendship request
         */
        ...
        const friendRequestExists = await t.selectFrom('friendships')
          .selectAll()
          .where('friendships.userId', "=", ctx.session.userId)
          .where('friendships.friendUserId', '=', input.friendUserId)
          .execute();
        ...
        if (friendRequestExists.length > 0) {
          return t.updateTable('friendships')
            .set({ status: FriendshipStatusSchema.Values['accepted'] })
            .where('friendships.userId', "=", ctx.session.userId)
            .where('friendships.friendUserId', '=', input.friendUserId)
            .executeTakeFirst();
          ...
          return t.insertInto('friendships')
            .values({
              userId: ctx.session.userId,
              friendUserId: input.friendUserId,
  ...
  decline: procedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      /**
       * Implement api to decline a friendship request
       */
